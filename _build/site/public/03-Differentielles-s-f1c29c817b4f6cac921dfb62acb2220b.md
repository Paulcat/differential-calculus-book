...
\end{equation*}
représentée par le gradient $f'(a)$, la différentielle seconde définit une application *bilinéaire*
\begin{equation*}
    (h,k) \mapsto f''(a) \cdot (h,k) = \sum_{i,j=1}^n \partial_{ij}^2 f(a) h_i k_k
\end{equation*}représentée par la matrice *hessienne* de $f$.
