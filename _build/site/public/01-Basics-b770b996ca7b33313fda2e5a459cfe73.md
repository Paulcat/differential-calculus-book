...
Dans la suite, on suppose que $E$ et $F$ sont tous deux de dimension finie (ce sera toujours le cas dans le cadre de ce cours), disons $n$ et $m$ respectivement. Soient $(e_1, \ldots, e_n)$ une base de $E$ (c'est-à-dire que tout vecteur de $E$ peut s'écrire de manière unique comme une combinaison linéaire des $e_i$) et $(v_1,\ldots,v_m)$ une base de $F$: c'est-à-dire que tout vecteur de $E$ (resp. F) se décompose de manière unique comme une combinaison linéaire des vecteurs $e_i$ (respectivement $v_j$).
