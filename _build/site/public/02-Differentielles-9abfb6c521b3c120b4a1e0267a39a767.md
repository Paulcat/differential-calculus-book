...
On considère maintenant une fonction $f:\mathbb{R}^n \to \mathbb{R}$. Pour $\mathbf{x} = (x_1?\ldots,x_n)^\top$ dans la base canonique, on notera indifféremment $f(\mathbf(x))$ ou $f(x_1,\ldots,x_n)$. La [différentiabilité](#def-diff) de $f$ entraîne l'existence des dérivées le long de chaque vecteur de base $e_i$, appelée *dérivées partielles*.

:::{prf:definition}
La $i$-ème dérivée partielle de $f$ en $a$, pour $i=1,\ldots,n$ est donnée par
\begin{equation*}
    \partial_i f(a) = f'(a)\cdot e_i = \lim_{t \to 0} \frac{f(a+ te_i)-f(a)}{t} = \lim_{t\to 0} \frac{f(a_1,\ldots,a_i+t,\ldots,a_n}{t}
\end{equation*}
:::
